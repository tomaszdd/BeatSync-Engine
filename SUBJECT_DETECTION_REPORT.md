...
**Layer 2 (Qwen) — now live-verified (2026-09-23).** Downloaded `llama-b9842-bin-win-vulkan-x64` + `Qwen3VL-2B-Instruct-Q8_0.gguf` (1.71GB) + `mmproj-Qwen3VL-2B-Instruct-F16.gguf` (781MB) from the official sources named in `scripts/install.ps1`, cached at `D:\BeatSync-Models\` on `um890` (persistent, outside the throwaway `C:\BeatSyncTest\` scratch env so it survives a rebuild) and copied into `bin/llama-bin-win-vulkan-x64/` + `bin/models/`. Re-ran the exact same test (`D:\Photos\GoPro\2025-10-01 Tereska Birth\`, same audio track) via `gui.process_video()` with the real backend present. Confirmed via the run log: `Qwen: enabled, model Qwen3VL-2B-Instruct-Q8_0 (llama.cpp Vulkan), batch 1`, 49 real Qwen tag calls completed (`Qwen tags: 49/235 in 2143.0s`), and 45/191 final clips in the output plan carry `ai_analyzed: true` with real categorical tags (`beauty`, `clean`, `drop`, `sad`, `soft`, etc.) — Layer 2 genuinely influenced clip selection, not just loaded inert. Output re-verified as genuine AMF hardware encode (`ffprobe` stream tag `Lavc63.1.102 h264_amf`, 3840x2160).

**Performance caveat found by this live test**: Stage 5 (visual analysis) took 2607s (~43.5 min) for this ~19.4-minute-footage / 235-candidate job — Qwen inference is the dominant cost at this quality setting (Q8_0, 2B params, iGPU Vulkan), vs. ~28s for beat detection and ~5.8s for final assembly. Total end-to-end: 2804s (~46.7 min), vs. seconds without Layer 2. Only 49/235 candidates actually got a live Qwen call before whatever budget/threshold governs Stage 5 moved on — worth checking `stage5_qwen_scene_worker.py`'s candidate-selection logic if full-coverage tagging matters for future runs; not investigated further here as it was out of scope for "is Layer 2 verified working."
...
2. **YOLOv8n can false-positive on extreme wide-angle/fisheye GoPro frames** at ~0.92 confidence (one confirmed instance in ~235 candidates). Layer 2 (Qwen) is the intended second opinion for exactly this case; now live-verified (see above), but this specific false-positive case wasn't individually re-checked against Layer 2's output.
